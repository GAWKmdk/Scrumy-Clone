 ...
 getListTask: function(storyId, state) {
  return TaskDB.findFaster({
   ...
   sort: {
    ...
    //code for displaying canvas types
        
   // , hasUserPersona: function(){
        // has to check current session
        // has to match current _id
        // has to find in collection type == 
   //return Collection.find({user_id: Meteor.userId()}).map(function (type) {
  //     return type._id;
  // })
  //  },
